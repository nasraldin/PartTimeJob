...
class BX_ajax_backend{
	...
	function init(){
		...
		add_action( 'wp_ajax_sort_gateways', array( $this, 'sort_gateways' ) );

	}
	function sort_gateways(){
		$string = '&'.$_POST['request'];
		//sort=stripe&sort=paypal&sort=mobilpay&sort=payfast&sort=cash&sort=paystack
		$gateways = explode("&sort=", $string);
		foreach ($gateways as $key => $name) {
			if(!empty($name)){
				box_update_gateway_possition($name, $key);
			}
	...
	static function create_package() {
		...
		if( isset( $request['zero_commision']) && $request['zero_commision'] == 'on' ){
			$args['meta_input']['zero_commision'] = 1;
		} else {
			$args['meta_input']['zero_commision'] = 0;
		}
		...
		if( $id ){
			if( isset($request['number_bids']) ){
				update_post_meta($id,'number_bids',$request['number_bids']);
			}
			// if( isset( $request['zero_commision']) && $request['zero_commision'] == 'on' ){
			// 	update_post_meta($id, 'zero_commision',$request['zero_commision']);
			// }
		}
	...
	function install_sample(){
		...
		$file = get_template_directory().'/sampledata/sample.xml';
